...
if __name__ == "__main__":
    ...
    with open(params.dataset.data_path, "r", encoding="utf-8") as f:
        lines = f.read().split("\n")[:-1]
    np.random.seed(params.seed)
    np.random.shuffle(lines)
    for line in lines[: min(params.dataset.num_samples, len(lines) - 1)]:
    ...
    arch = LSTMSeqToSeq(
        ...
        optim_params=OmegaConf.to_container(params.model.optim),
    )
